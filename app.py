...
def draw_action_map(df, title, top_n_highlight=20, offset_step=1.5, selected_action_num=None):
    ...
    def _is_sel(row):
        if selected_action_num is None:
            return False
        return int(row['number']) == int(selected_action_num)

    def draw_row(row, pos):
        is_selected = _is_sel(row)
        ...
        _draw_comet(ax, ox0, oy0, ox1, oy1,
                    color=color, alpha=alpha, lw_scale=lw_scale,
                    seg_alpha_factors=seg_alpha_factors[pos])
        return layer

    draw_order = sorted(
        range(len(df)),
        key=lambda i: (int(_action_visual(df.iloc[i], pos_ref, _is_sel(df.iloc[i]))[3]), float(scores[i]))
    )
    ...
    legend_items = [
        ...
        Line2D([0], [0], color='#aab2be', lw=2.3,
               ...
               label='Failed'),
        Line2D([0], [0], color='#00ffff', lw=3.5,
               marker='h', markersize=10, markerfacecolor='#00ffff',
    ...
    legend = ax.legend(handles=legend_items, loc='upper center', bbox_to_anchor=(0.5, -0.095),
                       ncol=4, frameon=True, facecolor='#1a1a2e', edgecolor='#6b6b8f',
    ...
    fig.subplots_adjust(left=0.02, right=0.90, top=0.94, bottom=0.12)
...
def render_top10_table(df, key_prefix='map'):
    """
    Renders the Top-10 ΔxT table.
    Returns (top_df, selected_row_index) where selected_row_index is an int or None.
    Does NOT write to session_state — the caller owns that.
    """
...
def plot_metric_line(metrics_df, key, label):
    ...
    ax.text(x.max() + 0.03, y_avg, f' avg: {y_avg:.2f}',
...
for key, default in [
    ...
    ('top_n_value', 10),
    ('selected_action', None),   # single source of truth: a pandas Series or None
    ('last_map_click', None),    # last raw click dict from streamlit_image_coordinates
]:
...
with tab_maps:
    ...
    # Reset selection when match/filter changes
    ...
    with col_main:
        ...
        with map_col:
            ...
            img_obj, ax, fig = draw_action_map(
                ...
                offset_step=1.5,
                selected_action_num=_sel_num,
            )

            click = streamlit_image_coordinates(img_obj, width=DISPLAY_WIDTH)
            plt.close(fig)

            # Process map click → update session_state['selected_action']
            if click is not None and click != st.session_state.get('last_map_click'):
                ...
                cands = df_sel[df_sel['_dist'] < 5.0]
                if not cands.empty:
                    st.session_state['selected_action'] = cands.sort_values('_dist').iloc[0]
        ...
        # ── Left: Video below the Action Map ─────────────────────────────────
        with map_col:
            st.markdown('<h4 style="color:#ffffff;margin:10px 0 4px 0;">Vídeo</h4>', unsafe_allow_html=True)
            if selected_action is None:
                st.info('Selecione um evento no mapa ou na tabela para ver o vídeo.')
            elif has_video_value(selected_action['video']):
                try:
                    st.video(selected_action['video'])
                except Exception:
                    st.error('Vídeo não encontrado.')
            else:
                st.warning('Sem vídeo disponível para este evento.')

        # ── Right: Event Panel below the Top-10 table ─────────────────────────
        with top10_col:
            st.markdown('<h4 style="color:#ffffff;margin:16px 0 4px 0;">Event Panel</h4>', unsafe_allow_html=True)
            if selected_action is None:
                st.info('Selecione um evento na tabela ou clique no mapa.')
            else:
                ...
                st.markdown(
                    ...
                    f'background:{act_color};border:2px solid #fff;"></span>'
                ...
                with c1:
                    ...
                    st.write(f'**Direction:** {str(selected_action["direction"]).capitalize()}')
